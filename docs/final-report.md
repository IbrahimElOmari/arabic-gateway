...
## Task Status Overview (Original 40 + Refinements + Phase 3)
...
| P3.5 | API wrapper uitrol (`apiQuery`/`apiMutate`) | ✅ | `supabase-api.ts`, `admin-log.ts`, `LevelsPage.tsx`, `StudentDashboard.tsx`, `PaymentsPage.tsx`, `EnrollmentRequestsPage.tsx` |
| P3.6 | Uniforme toast strategie | ✅ | Alle 20+ bestanden gemigreerd van `toast()` → `useToast()` hook |
...
- **Backend:** Lovable Cloud (30+ tabellen, 16 edge functions, RLS op alle tabellen)
...
- **Notificaties:** Real-time via Supabase Realtime + `notifications` tabel
- **Error handling:** `apiQuery`/`apiMutate` wrappers (15s timeout, 1x retry 5xx)
...
| Testing | 70% |
| Code Coherentie | 95% |
| **Gewogen gemiddelde** | **~88%** |

Gap tot 100%: voornamelijk externe configuratie (Stripe/Resend/pg_cron) en verdere E2E test coverage.
