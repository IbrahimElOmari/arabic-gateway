...
export default function PricingPage() {
  ...
  const { data: classes, isLoading } = useQuery({
    ...
    queryFn: () => apiQuery<any[]>("classes", (q) =>
      q.select(`id, name, description, price, currency, max_students, start_date, end_date, is_active, levels (name_nl, name_en, name_ar)`)
    ...
    <>
      <div className="container py-12">
        <div className="mx-auto max-w-3xl text-center mb-10">
          ...
          <p className="text-muted-foreground text-lg">{t("pricing.subtitle", "Kies de cursus die bij je past en begin vandaag nog met Arabisch leren.")}</p>
        </div>
        ...
        <div className="mx-auto max-w-md mb-8">
          ...
          {appliedDiscount && (
            <p className="text-sm text-success mt-2 flex items-center gap-1">
              ...
              {appliedDiscount.type === "percentage" ? `${appliedDiscount.value}%` : formatCurrency(appliedDiscount.value, "EUR", locale)}
            </p>
          )}
        </div>
        ...
        {isLoading ? (
                ...
                <Card key={cls.id} className="flex flex-col">
                  ...
                  <CardContent className="flex flex-1 flex-col justify-between gap-4">
                    ...
                    <div>
                      {hasDiscount ? (
                        <div className="mb-3">
                          <p className="text-lg line-through text-muted-foreground">{formatCurrency(originalPrice!, cls.currency || "EUR", locale)}</p>
                          <p className="text-2xl font-bold text-success">{formatCurrency(discountedPrice!, cls.currency || "EUR", locale)}</p>
                        </div>
                      ) : (
                        <p className="text-2xl font-bold mb-3">{originalPrice ? formatCurrency(originalPrice, cls.currency || "EUR", locale) : t("pricing.free", "Gratis")}</p>
                      )}
                      {taxInfo && originalPrice ? <p className="text-xs text-muted-foreground mb-2">{t("pricing.inclVat", "Incl. {{rate}}% BTW", { rate: taxInfo.vatRate * 100 })}</p> : null}
                      <Button className="w-full" onClick={() => handleEnroll(cls.id, isFree)} disabled={!!enrollmentStatus || enrollMutation.isPending}>
                        {enrollMutation.isPending ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <GraduationCap className="h-4 w-4 mr-2" />}
                        {enrollmentStatus ? (enrollmentStatus === "enrolled" ? t("pricing.alreadyEnrolled", "Al ingeschreven") : t("pricing.pendingApproval", "Wacht op goedkeuring")) : t("pricing.enroll", "Inschrijven")}
                      </Button>
                    </div>
                  </CardContent>
                </Card>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">{t("pricing.noClasses", "Er zijn momenteel geen actieve cursussen.")}</p>
          </div>
        )}
      </div>
    </>
