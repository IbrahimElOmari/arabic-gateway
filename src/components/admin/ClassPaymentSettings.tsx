...
export function ClassPaymentSettings({ classId, className, currentPrice, currency }: ClassPaymentSettingsProps) {
  ...
  const { data: discounts, isLoading } = useQuery({
    ...
    queryFn: () => apiQuery<any[]>("discount_codes", (q) => q.select("*").eq("class_id", classId).order("created_at", { ascending: false })),
  });
  ...
  const createDiscountMutation = useMutation({
    mutationFn: (data: typeof discountForm) =>
      apiMutate("discount_codes", (q) => q.insert([{
        code: data.code.toUpperCase(), class_id: classId,
        discount_type: data.discount_type as "percentage" | "fixed_amount",
        discount_value: parseFloat(data.discount_value),
        valid_until: data.valid_until || null,
        max_uses: data.max_uses ? parseInt(data.max_uses) : null,
        is_active: data.is_active, created_by: user!.id,
      }])),
    onSuccess: () => {
    ...
    onError: (error: any) => {
      toast({ variant: "destructive", title: t("common.error"),
        description: error.message?.includes("duplicate") ? t("admin.codeExists", "This code already exists") : undefined });
    },
  });

  const deleteDiscountMutation = useMutation({
    mutationFn: (id: string) => apiMutate("discount_codes", (q) => q.delete().eq("id", id)),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["class-discounts", classId] });
  ...
  const toggleDiscountMutation = useMutation({
    mutationFn: ({ id, isActive }: { id: string; isActive: boolean }) =>
      apiMutate("discount_codes", (q) => q.update({ is_active: isActive }).eq("id", id)),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["class-discounts", classId] }); },
  });

  const resetDiscountForm = () => {
    setDiscountForm({ code: "", discount_type: "percentage", discount_value: "", valid_until: "", max_uses: "", is_active: true });
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2"><Percent className="h-5 w-5" />{t("admin.discountCodes", "Discount Codes")}</CardTitle>
              <CardDescription>{t("admin.discountCodesDescription", "Create discount codes for this class")}</CardDescription>
            </div>
            <Button onClick={() => { resetDiscountForm(); setShowDiscountDialog(true); }}><Plus className="h-4 w-4 mr-2" />{t("admin.addDiscount", "Add Discount")}</Button>
          </div>
        </CardHeader>
        <CardContent>
          {isLoading ? (
              ...
              <TableBody>
                {discounts.map((discount) => (
                  <TableRow key={discount.id}>
                    ...
                    <TableCell>{discount.discount_type === "percentage" ? `${discount.discount_value}%` : `€${discount.discount_value}`}</TableCell>
                    <TableCell>{discount.current_uses}/{discount.max_uses || "∞"}</TableCell>
                    <TableCell>{discount.valid_until ? formatDate(discount.valid_until, "PP") : t("admin.noExpiry", "No expiry")}</TableCell>
                    <TableCell><Switch checked={discount.is_active} onCheckedChange={(checked) => toggleDiscountMutation.mutate({ id: discount.id, isActive: checked })} /></TableCell>
                    <TableCell className="text-right"><Button variant="ghost" size="icon" onClick={() => deleteDiscountMutation.mutate(discount.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
          ) : (
            <div className="text-center py-6 text-muted-foreground">{t("admin.noDiscounts", "No discount codes for this class")}</div>
          )}
        </CardContent>
      </Card>
      ...
      <Dialog open={showDiscountDialog} onOpenChange={setShowDiscountDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{t("admin.createDiscount", "Create Discount Code")}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div>
              <Label>{t("admin.discountCode", "Discount Code")}</Label>
              <Input value={discountForm.code} onChange={(e) => setDiscountForm({ ...discountForm, code: e.target.value.toUpperCase() })} placeholder="SAVE20" className="font-mono uppercase" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>{t("admin.discountType", "Type")}</Label>
                <Select value={discountForm.discount_type} onValueChange={(v: "percentage" | "fixed_amount") => setDiscountForm({ ...discountForm, discount_type: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div>
                <Label>{discountForm.discount_type === "percentage" ? t("admin.percentageValue", "Percentage (%)") : t("admin.amount", "Amount (€)")}</Label>
                <Input type="number" min={0} max={discountForm.discount_type === "percentage" ? 100 : undefined} value={discountForm.discount_value} onChange={(e) => setDiscountForm({ ...discountForm, discount_value: e.target.value })} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <Input type="number" min={1} value={discountForm.max_uses} onChange={(e) => setDiscountForm({ ...discountForm, max_uses: e.target.value })} placeholder={t("admin.unlimited", "Unlimited")} />
              </div>
            </div>
            <div className="flex items-center justify-between">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowDiscountDialog(false)}>{t("common.cancel", "Cancel")}</Button>
            <Button onClick={() => createDiscountMutation.mutate(discountForm)} disabled={createDiscountMutation.isPending}>
              {createDiscountMutation.isPending && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}{t("common.create", "Create")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
